...
public class SelectionManager : MonoBehaviour
{
    ...
    public void SetSelectedObject(int instanceID)
    {
        ...
        if (_selectedObjectID != instanceID)
        {
            ...
            if (instanceID > -1 )
            {
                if (_ctrlKeyFlag)
                {
                    float radius = SceneManager.Get.ProteinRadii[(int)SceneManager.Get.ProteinInstanceInfos[instanceID].x] * PersistantSettings.Get.Scale;

                    SelectionGameObject.GetComponent<SphereCollider>().radius = radius;

                    SelectionGameObject.transform.position = SceneManager.Get.ProteinInstancePositions[instanceID] * PersistantSettings.Get.Scale;
    ...
    private void UpdateSelectedElement()
    {
        ...
        if (_selectionGameObject.transform.hasChanged)
        {
            ...
            SceneManager.Get.ProteinInstancePositions[_selectedObjectID] = _selectionGameObject.transform.position / PersistantSettings.Get.Scale;
