...
namespace UIWidgets
{
		...
		where TItem: IItemHeight
	{
		...
		protected override void ScrollTo(int index)
		{
			...
			var item_ends = ItemEndAt(index) + margin;
		...
		protected override float CalculateBottomFillerSize()
		{
			...
			return Mathf.Max(0, height + (layout.Spacing.y * (bottomHiddenItems - 1)));
		}
		...
		protected override float CalculateTopFillerSize()
		{
			...
			return Mathf.Max(0, height + (layout.Spacing.y * (topHiddenItems - 1)));
		}
