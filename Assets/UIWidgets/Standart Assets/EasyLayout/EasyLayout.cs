...
namespace EasyLayout {
	...
	public class EasyLayout : UnityEngine.UI.LayoutGroup {
		...
		List<float> GetRowsWidths(List<List<RectTransform>> group)
		{
			...
			{
				var width = group[row_index].ConvertAll<float>(ScaledWidth).Sum();
		...
		{
			return column.ConvertAll<float>(ScaledWidth).Max();
		}
		...
		List<float> GetColumnsHeights(List<List<RectTransform>> group)
		{
			...
			{
				var height = group[column_index].ConvertAll<float>(ScaledHeight).Sum();
		...
		{
			return Transpose(group).Select<List<RectTransform>,float>(GetMaxRowHeight).ToArray();
		}

		Vector2 GetMaxCellSize(List<List<RectTransform>> group)
		{
			var rows_max = group.ConvertAll<Vector2>(GetMaxCellSize);
		...
		public static float GetPreferredWidth(RectTransform rect)
		{
			...
			float result = 1f;
			foreach (var elem in rect.GetComponents<ILayoutElement>())
			{
		...
		List<RectTransform> GetUIElements()
		{
			...
			if (Filter!=null)
			{
				var temp = Filter(elements.ConvertAll<GameObject>(GetGameObject));
		...
		void ResizeColumnWidthToFit(float width, List<List<RectTransform>> group)
		{
			...
			var row_preferred_widths = transpore_group.ConvertAll<float>(GetMaxPreferredWidth);
			var row_flexible_widths = transpore_group.ConvertAll<float>(GetMaxFlexibleWidth);
		...
		{
			var row_preferred_heights = group.ConvertAll<float>(GetMaxPreferredHeight);
			var row_flexible_heights = group.ConvertAll<float>(GetMaxFlexibleHeight);
