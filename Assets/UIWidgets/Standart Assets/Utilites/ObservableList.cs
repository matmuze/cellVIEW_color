...
namespace UIWidgets {
	...
	public class ObservableList<T> : IObservableList<T>, IList<T> {
		...
		void AddCallbacks(IEnumerable<T> items)
		{
			...
			{
				items.ForEach(x => (x as IObservable).OnChange += Changed);
			}
		...
		void AddCallback(T item)
		{
			if (IsItemsObservable)
			{
				(item as IObservable).OnChange += Changed;
			}
			else if (IsItemsSupportNotifyPropertyChanged)
			{
		...
		/// -or-
		/// null to use the default comparer Comparer<T>.Default.</param>
		...
		{
			return new ObservableList<TOutput>(Items.ConvertAll(converter));
		}
		...
		{
			return new ObservableList<T>(Items.FindAll(match));
		}
		...
		{
			return new ObservableList<T>(Items.GetRange(index, count));
		}
